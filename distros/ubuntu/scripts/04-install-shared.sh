...
if ls /usr/local/share/zsh/site-functions/*.zsh >/dev/null 2>&1; then
  ...
  install -m 644 /opt/wsl-cloud-init/shared/zsh/*.zsh /usr/local/share/zsh/site-functions/
fi
...
if ls "/home/$TARGET_USER/.config/direnv/lib/"*.sh >/dev/null 2>&1; then
  ...
  install -o "$TARGET_USER" -g "$TARGET_USER" -m 644 /opt/wsl-cloud-init/shared/direnv/lib/*.sh "/home/$TARGET_USER/.config/direnv/lib/"
fi
...
if [[ -n "$(ls -A "/home/$TARGET_USER/.claude/skills" 2>/dev/null)" ]]; then
  ...
  sudo -u "$TARGET_USER" cp -r /opt/wsl-cloud-init/shared/claude/skills/. "/home/$TARGET_USER/.claude/skills/"
fi
