...
- **Git, the easy way** *(opt-in)* — your Git identity, `gh` auth, and git authenticating through `gh` (one shared credential), plus `clone-repo` / `create-repo` / `create-branch` / `update-branch` helpers.
