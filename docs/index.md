...
- **Claude Code** *(opt-in: `-InstallClaudeCode`)* — the CLI pre-wired to the Context7 MCP for up-to-date library docs, plus an `add-sonarqube-mcp` helper to enable the SonarQube Cloud MCP per project (great for private repos).
