...
└── system/                                 # → /   (root-owned)
    └── usr/local/
        ...
        └── share/
            ├── zed/*.json                          → /usr/local/share/zed/ (then seeded to Windows)
            └── zsh/site-functions/
...
| `system/usr/local/share/zed/*.json` | `/usr/local/share/zed/` → Windows `%APPDATA%\Zed\` | `distros/ubuntu/scripts/15-install-zed-interop.sh` | `INSTALL_ZED_INTEROP` |
