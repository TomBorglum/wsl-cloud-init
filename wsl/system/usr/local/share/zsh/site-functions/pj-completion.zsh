...
# version only completes the project name in its actual argument slot, and
# never offers the `open` subcommand itself — `pjo` is the shorthand for it.
_pj() {
  local -a projects
  local basedir
  ...
  case $CURRENT in
    2) compadd -a projects ;;                           # pj <project>
